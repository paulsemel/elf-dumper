...
static char *strtab_resolve(ElfW(Shdr) * sh, ElfW(Half) shnum,
			    ElfW(Half) shentsize, char *strtab, char *base,
			    const char *section)
{
	char *str = NULL;
	for (int i = 0; i < 5; i++) {
		ElfW(Shdr) *tmp =
			get_shdr_entry(sh, shnum, shentsize, SHT_STRTAB, i);
...
void print_symbol(ElfW(Sym) * sym, ElfW(Xword) symnum, ElfW(Xword) symsize,
		  char *strtab)
{
	char value[256];
	for (ElfW(Xword) i = 0; i < symnum;
	     i++, sym = (ElfW(Sym) *)((char *)sym + symsize)) {
		new_section(NULL);
		   ...
		   char *strtab, char *base)
{
	...
	char *dynstr =
		strtab_resolve(sh, shnum, shsize, strtab, base, ".dynstr");
	print_symbol((ElfW(Sym) *)(save->sh_offset + base),
		     save->sh_size / sizeof(ElfW(Sym)), sizeof(ElfW(Sym)),
		     dynstr);
	end_array("data", true);
